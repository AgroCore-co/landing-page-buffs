...
export default function Header({ isHeaderScrolled, isMobileMenuOpen, toggleMobileMenu }) {
  ...
  const toggleDropdown = (menuName) => {
    setOpenMobileDropdown((previous) => (previous === menuName ? null : menuName));
  };
  ...
  return (
    <>
      {/* Header / Navigation */}
      <nav
        className={`fixed top-0 w-full z-50 py-2 md:py-3 transition-all duration-200 ${
          isHeaderScrolled
            ? 'bg-[#FDFBF7]/92 backdrop-blur-xl border-b border-black/5 shadow-sm'
            : 'bg-[#FDFBF7]/82 backdrop-blur-xl border-b border-black/5'
        }`}
      >
        <div className="mx-auto w-full max-w-[1800px] px-4 sm:px-6 lg:px-10 flex items-center justify-between">
          {/* Logo Buffs */}
          <Link href="/" className="flex items-center gap-2" aria-label="Ir para a home">
            <svg className="h-8 md:h-9 w-auto" viewBox="0 0 491 159" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Menu Mobile Overlay */}
      <div className={`fixed inset-0 bg-white z-[100] transform ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-500 ease-in-out flex flex-col pt-20 sm:pt-24 px-6 sm:px-8 pb-6 sm:pb-8 lg:hidden`}>
        <button onClick={handleMenuToggle} className="absolute top-6 right-6 sm:top-8 sm:right-8 text-[#43310B] p-2 focus:outline-none hover:rotate-90 transition-transform duration-300 cursor-pointer">
        ...
        <div className="flex flex-col gap-5 text-xl sm:text-2xl font-bold text-[#43310B] mt-8 sm:mt-10">
          ...
          <div className="border-b border-black/10 pb-4">
            <button onClick={() => toggleDropdown('contato')} className="w-full inline-flex items-center justify-between hover:text-[#FCA90F] transition-colors">
              Contato
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`transition-transform duration-200 ${openMobileDropdown === 'contato' ? 'rotate-180' : ''}`}>
          ...
          <div className="border-b border-black/10 pb-4">
            <button onClick={() => toggleDropdown('institucional')} className="w-full inline-flex items-center justify-between hover:text-[#FCA90F] transition-colors">
              Institucional
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`transition-transform duration-200 ${openMobileDropdown === 'institucional' ? 'rotate-180' : ''}`}>
                <polyline points="6 9 12 15 18 9"></polyline>
              </svg>
            </button>
